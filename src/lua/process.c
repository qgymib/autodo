...
wakeup_host_co:
    auto_api()->coroutine.set_state(process->host_co, LUA_TNONE);
}
...
static int _lua_process_gc(lua_State *L)
{
    ...
    {
        auto_api()->process.kill(process->process, 9);
...
static int _lua_process_kill(lua_State *L)
{
    ...
    if (process->process != NULL)
    {
        auto_api()->process.kill(process->process, signum);
        process->process = NULL;
    }
...
static int _lua_process_on_stdout_resume(lua_State *L, int status, lua_KContext ctx)
{
    ...
    {
        auto_api()->coroutine.set_state(process->host_co, LUA_YIELD);
...
int atd_lua_process(lua_State *L)
{
    ...
    process->host_co = auto_api()->coroutine.find(L);
    ...
    if ((process->process = auto_api()->process.create(&process->cfg)) == NULL)
    {
        lua_pop(L, 1);
        return 0;
    }
