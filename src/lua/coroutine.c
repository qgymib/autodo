...
static int _coroutine_gc(lua_State* L)
{
    ...
    if (co->hook != NULL)
    {
        auto_api()->coroutine.unhook(co->thr, co->hook);
        co->hook = NULL;
    }
...
static void _on_coroutine_state_change(atd_coroutine_t* coroutine, void* arg)
{
    ...
    {
        auto_api()->coroutine.set_state(co->await_thr, LUA_TNONE);
    }
...
static int _coroutine_on_resume(lua_State *L, int status, lua_KContext ctx)
{
    ...
    /* Add to wait_queue */
    auto_api()->coroutine.set_state(co->await_thr, LUA_YIELD);
...
{
    lua_coroutine_t* co = lua_touserdata(L, 1);
    co->await_thr = auto_api()->coroutine.find(L);
...
int atd_lua_coroutine(lua_State *L)
{
    ...
    /* Initialize */
    co->thr = auto_api()->coroutine.host(lua_newthread(L)); lua_pop(L, 1);
    ...
    co->hook = auto_api()->coroutine.hook(co->thr, _on_coroutine_state_change, co);
